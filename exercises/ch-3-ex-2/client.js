...
var refresh_token = "j2r3oj32r23rmasd98uhjrk2o3ixxxxxxxxxx";
...
app.get("/callback", function (req, res) {
  ...
  if (tokRes.statusCode >= 200 && tokRes.statusCode < 300) {
    ...
    if (body.refresh_token) {
      ...
      console.log("No refresh token found: ", JSON.stringify(body));
    }
...
app.get("/fetch_resource", function (req, res) {
  ...
  if (resource.statusCode >= 200 && resource.statusCode < 300) {
    ...
    if (refresh_token) {
      ...
      console.log(
        "no refresh token; resource status error code " + resource.statusCode
      );
      res.render("error", {
...
var refreshAccessToken = function (req, res) {
  ...
  if (tokRes.statusCode >= 200 && tokRes.statusCode < 300) {
    ...
    console.log("refresh token response body: ", JSON.stringify(body));
    access_token = body.access_token;
    if (body.refresh_token) {
      refresh_token = body.refresh_token;
      ...
      refresh_token = null;
      console.log("refresh token doen't work: ", JSON.stringify(body));
      res.render("error", { error: "refresh token doen't work: " });
    }
  } else {
    refresh_token = null;
    console.log("refresh token doen't work: ", JSON.stringify(tokRes));
